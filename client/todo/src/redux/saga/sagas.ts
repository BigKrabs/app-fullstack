...
import { ICreateAction, IDeleteAction, ITodo, ITodoActionTypes, ICompleteAction, IEditAction } from './../../types/types';
...
function* sagaGetTodos(): Generator<Effect, void, ITodo[]> {
    try {
        ...
        yield put(showAlert('He удалось загрузить дела', 'warning'));
    }
}
...
function* sagaCreateTodo(action: ICreateAction): Generator<Effect, void> {
    try {
        ...
        yield put(showAlert('He удается создать дело', 'warning'));
    }
}
...
function* sagaCompleteTodo(action: ICompleteAction<ITodo>): Generator<Effect, void> {
    try {
        ...
        yield call(TodoApi.comleteTodo, action.payload);
        ...
        yield put(showAlert(`He удалось завершить дело: ${error}`, 'warning'));
    }
}
...
function* sagaDeleteTodo(action: IDeleteAction): Generator<Effect, void> {
    try {
        ...
        yield put(showAlert(`He удалось удалить дело: ${error}`, 'warning'));
    }
}
...
function* sagaEditTodo(action: IEditAction): Generator<Effect, void, ITodo> {
    try {
        ...
        yield put(showAlert(`He удалось изменить дело: ${error}`, 'warning'));
    }
}
